...
<details>
  ...
  <br>
   ...
   > 5. Данные переданы на траснпортный уровень, передача данных с помощью протокола TCP.
   > При передаче данные разбиваются на ***сегменты***, к каждому сегменту добавляются ***заголовки*** со служебной информацией. Указываются **логические порты получателя и отправителя**.
