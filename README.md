...
* ### [Основы сетевой безопасность](#title7)
...
<details>
  ...
  1 из возможных вариантов защиты сети - **эшелированная оборона**. Использует многоуровневый подход к защите:
...
<details>
  ...
  1. Неавторизованный физический доступ. Решение: физическое ограничение доступа: замки, контроль доступа в помещение, ...

  2. Неавторизованный сетевой доступ. Решение: пароли, централизованная аутентификация.

  3. Использование незащищенных протоколов управления. Решение: SSH, HTTPS, SNMPv3.

  4. Неправильное обслуживание. Решение: разработать и строго соблюдать регламент конфигурирования оборудования.
</details>

<details>
  <summary>Контрмеры угрозам сетевого взаимодейтсвия (контрмеры, примеры)</summary>
  <br>

   1. Контроль подключения к сети. Примеры: Portsecurity, 802.1x, WPA.

   2. Контроль подключенных устройств. Примеры: DHCP snooping, Dynamic ARP Inspection, IP Source Guard.

   3. Блокировка нежелательного трафика. Примеры: IP ACL, Zone based firewall, Unicast Reverse Path Forwarding/

   4. Защита передаваемого трафика. Примеры: IPSec, SSL VPN, HTTPS.

   5. Инспекция пакетов. Примеры: Signature-based, Anomaly-based, Policy-based.
</details>
...
<details>
  ...
  1. **Фильтрация пакетов без отслеживания соединений** (траснпортный и сетевой; рассматривает заголовки кажждого пакета независимо от других пакетов).

  2. **Фильтрация пакетов с отслеживанием соединений** (транспортный и сетевой; рассматривает заголовки на соответствие состоянию сессии (инкапсуляция/декапсуляция)).

  3. **Фильтрация пакетов с отслеживанием соединений, инспекцией и контролем приложения** (прикладной, транспортный и сетевой; блокирвует вышрузку файло на сервер, но разрешает загрузку с него).

  Дополнительно:

  1. **Предотвращение вторжений** (прикладной, транспортный и сетевой; база данных сигнатур SIG 1234-HTTP CMD.EXE access (http://server/scripts/..//system32/**cmd.exe**?/c+dir+c)). Вредосноный URL, предназначенный для атаки на веб-сервер.

  2. **Анализ сетевого поведения** (прикладной, транспортный и сетевой; отслеживает аномальное поведение трафика (со стороны ПК 100 ICMP/sec)).

  3. **Шлюз для приложений** (прикладной, транспортный и сетевой; фильтрация Javascript с помощью HTTP ALG (фильтрация запроса, выдаст только index.html)).

  4. **VPN** (remote acess vpn - подключение через зашифрованный туннель; site-to-site vpn - создаёт туннель).
