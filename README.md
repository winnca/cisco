...
* ### [Основы сетевой безопасности](#title7)
...
<details>
  ...
  **Firewall** — система безопасности (аппаратная или программная), которая контролирует и фильтрует сетевой трафик между двумя сетями. Например, между вашей домашней сетью и интернетом, блокируя нежелательные или опасные соединения и пропуская разрешённые.
  ...
  <details>
    <summary>Firewall аппаратный</summary>
    <br>
    <img width="51" height="91" alt="image" src="https://github.com/user-attachments/assets/132af9a9-5e4b-4597-bc1a-3c4157f05da1" />
  </details>

  **Точка доступа** — устройство, позволяющее беспроводным устройствам (смартфонам, ноутбукам) подключаться к сети Wi-Fi. Действует, как мост, передавая данные между беспроводными клиентами и существующей проводной сетью, используя радиоволны, а не кабели.
</details>
...
<details>
  ...
  > Раньше использовалось деление на классы, но вскоре их отменили из-за большое количества пользователей в интернете, на смену пришла CIDR. Используются **префиксы** (об этом в масках).
...
## <a id="title7">Сетевая безопасность</a>

<details>
  <summary>Задача</summary>
  <br>

  Сети раскрывают ресурсы широкому кругу лиц, в том числе потенциальным атакующим. Из-за этого уязвимы.

  Задача заключается в том, чтобы обеспечить 3 важнейших сервиса ***для управления рисками***:
...
<details>
  ...
  * **Актив** - то, что имеет значение для организации.

  * **Угроза** - потенциальная опасность для ситемы/потенциальное негативное событие. Пример: хакерская атака.
  
  * **Уязвимость** - слабость в системе, которой можно воспользоваться. То есть позволяет угрозе развиваться. Пример: незащищённый порт.

  * **Риск** - вероятность того, что ***угроза будет реализована через узявимость***. Пример: риск утечки данных.

  * **Контрмеры** – действия по устранению уязвимостей и рисков (для защиты систем от угроз). Пример: правильно настроенный firewall.
</details>

<details>
  <summary>Классификация активов</summary>
  <br>

  1. Не все активы одинаковы ценны.
  
  2. Цель - обеспечить **комбинацию целостности, доступности, кондифициальности соответствующих ценности актива**.
  
  3. Должна соответствовать закону.

  Из +, определяет:

  * обязательства по защите активов
...
<details>
  <summary>Причины уязвимостей</summary>
  <br>
...
<details>
  <summary>Классификация контрмер</summary>

  1. административные, физические, технические
      ...
      * **Технические**:
          ...
          * Система обнаржуния (IDS) и предовтращения вторжений (IPS)
       
          * Бэкапы (резервное копирование данных)
       
          * Защита утечки данных (DLP)
      ...
      * **Административные**:
          ...
          * Процедуры управления доступом и правами пользователей

          * Регулярные проверки и аудиты безопасности

          * Обучение персонала основам кибербезопасности и осведомленности об угрозах

          * Планы реагирования на инциденты 

      * Физические:

          * Контроль доступа в помещения, где расположено сетевое оборудование

          * Системы видеонаблюдения

          * Физическая защита серверов и рабочих станций (например, от кражи)

          * Меры по предотвращению пожаров и других стихийных бедствий 
  
  2. Превентивные, реактивные, детективные

  3. Предотвращение, уменьшение, передача, принятие
</details>

<details>
  <summary>Основные этапы атаки</summary>

  1. Разведка (анализ следов).

  2. Перечисление приложений и ОС.
  ...
  6. Установка "чёрного хода".

  7. Использование скомпроментированной системы.
</details>

<details>
  <summary>Классификация угроз</summary>
  <br>

  1. **Вирусы, черви, трояны** - вредоносные программы, которые проникают в компьютер или сеть и наносят различный вред. С их помощью киберпреступники крадут, портят и уничтожают данные.

  2. **Фишинг** — мошенническая практика, когда киберпреступники выдают себя за надежные источники для получения личной информации. Одна из разновидностей фишинга — использование подменных доменных имен, которые похожи на настоящие.

  3. **Ботнет** — сеть зараженных компьютеров, используемых для выполнения вредоносных действий без ведома их владельцев.

  4. **Эксплойты** — уязвимости в ПО и аппаратной части, которые киберпреступники могут использовать для взлома. К эксплойтам относятся, в частности, программные и аппаратные бэкдоры.

        * **Бэкдор** ("задняя дверь") — это скрытый способ обхода обычных мер безопасности, который позволяет получить несанкционированный доступ к компьютерной системе, сети или устройству.  

            * Этот метод может быть вредоносным ПО или уязвимостью, созданной намеренно или случайно, и позволяет злоумышленникам получить полный контроль, доступ к данным или удалённое управление.

  5. **SQL-инъекции** — метод атаки, при котором злоумышленники внедряют SQL-код в запросы к базе данных для получения доступа к данным, содержащимся в этой БД.

  6. **Скрытые загрузки** — загрузка вредоносного кода на компьютер без ведома пользователя.

  7. **Перехват паролей** — процесс получения доступа к чужому аккаунту путем кражи учетных данных.

  8. **DDoS** (Distributed Denial of Service) — атака, направленная на перегрузку сети или конкретного сервера большим количеством запросов. DDoS-атаки приводят к временной недоступности инфраструктуры.

  9. **Социальная инженерия** — манипуляции людьми с целью получения личной информации или выполнения определенных действий. Например, злоумышленник может различными способами втираться в доверие к жертве, ведя с ней переписку в социальных сетях или мессенджерах.

  10. ...  
</details>

<details>
  <summary>Варианты защиты сети</summary>

  1. Эшелированная оборона.

       * использует многоуровневый подход к защите:
  ...
  > Например, запрос на доступ к файлу. Запрос проходит через 2 firewall (вначале через "пограничный", затем через "хост"), потом стучиться к файл-серверу и получает доступ.

  2. Обособление.

  3. Принцип наименьших привилегий.

  4. Поиск слабых звеньев.

  5. Разделение и ротация обязанностей.

  6. Опосредованный доступ.

  7. Учет и отслеживание.
</details>

<details>
  <summary>Контрмеры угрозам сетевого устройства (угроза, контрмера)</summary>
  <br>

  1. Неавторизованный физический доступ.

      * Решение: физическое ограничение доступа: замки, контроль доступа в помещение, ...

  2. Неавторизованный сетевой доступ.

      * Решение: пароли, централизованная аутентификация.

  3. Использование незащищенных протоколов управления.

      * Решение: SSH, HTTPS, SNMPv3.

  4. Неправильное обслуживание.

      * Решение: разработать и строго соблюдать регламент конфигурирования оборудования.
</details>

<details>
  <summary>Контрмеры угрозам сетевого взаимодейтсвия (контрмера, пример)</summary>
  <br>

   1. Контроль подключения к сети.

      * Примеры: Portsecurity, 802.1x, WPA.

   2. Контроль подключенных устройств.

      * Примеры: DHCP snooping, Dynamic ARP Inspection, IP Source Guard.

   3. Блокировка нежелательного трафика.

      * Примеры: IP ACL, Zone based firewall, Unicast Reverse Path Forwarding/

   4. Защита передаваемого трафика.

      * Примеры: IPSec, SSL VPN, HTTPS.

   5. Инспекция пакетов.

       * Примеры: Signature-based, Anomaly-based, Policy-based.
</details>

<details>
  <summary>Задачи firewall</summary>
  <br>

  Основные:

  1. **Фильтрация пакетов без отслеживания соединений**.

      * траснпортный и сетевой; рассматривает заголовки кажждого пакета независимо от других пакетов.

  2. **Фильтрация пакетов с отслеживанием соединений**.

      * транспортный и сетевой; рассматривает заголовки на соответствие состоянию сессии (инкапсуляция/декапсуляция).

  3. **Фильтрация пакетов с отслеживанием соединений, инспекцией и контролем приложения**.

      * прикладной, транспортный и сетевой; блокирвует вышрузку файло на сервер, но разрешает загрузку с него.

  Дополнительно:

  1. **Предотвращение вторжений**.

      * прикладной, транспортный и сетевой; база данных сигнатур SIG 1234-HTTP CMD.EXE access ("ht...://server/scripts/..//system32/**cmd.exe**?/c+dir+c"). Вредосноный URL, предназначенный для атаки на веб-сервер.

  2. **Анализ сетевого поведения**.

      * прикладной, транспортный и сетевой; отслеживает аномальное поведение трафика (со стороны ПК 100 ICMP/sec).

  3. **Шлюз для приложений**.

      * прикладной, транспортный и сетевой; фильтрация Javascript с помощью HTTP ALG (фильтрация запроса, выдаст только index.html).

  4. **VPN**.

      * remote acess vpn - подключение через зашифрованный туннель; site-to-site vpn - создаёт туннель).

  <img width="1107" height="399" alt="image" src="https://github.com/user-attachments/assets/9fc04b6b-5cad-4602-9de0-4aa69bf983b0" />
</details>
...
<details>
  ...
  * **IDS** - система обнаружения вторжений. Обнаруживает енправомерное использование сетевых ресурсов, а также доступ к ним.

  <details>
    <summary>Иллюстрация</summary>
    <br>
    <img width="796" height="396" alt="image" src="https://github.com/user-attachments/assets/483b82fe-d41e-4ff6-bf9f-adb2ee97cac5" />
  </details>

  * **IPS** - система предотвращения вторжений. Обнаруживает и предотвращает неправомерное использование сетевых ресурсов и несанкционированный доступ к ним.

  <details>
    <summary>Иллюстрация</summary>
    <br>
    <img width="825" height="369" alt="image" src="https://github.com/user-attachments/assets/1daea5fb-d9e6-4978-833b-ddaa96210f87" />
  </details>
</details>

<details>
  <summary>Реакции системы безопасности IDS/IPS</summary>
  <br>

  * **Истинно-позитивный**. СБ сработала в результате вредосной активности.

  * **Ложно-позитивный**. СБ сработало ложно в резултате безобидных действий.

  * **Истинно-не...**. СБ не сработало, поскольку вредоносных действий не было.

  * **Ложно-не...**. СБ не сработало несмотря на наличие вредоносной активности.
</details>

<details>
  <summary>Режимы работы IPS</summary>
  <br>

  1. **Signature-Based IPS** (на основе сигнатур).

      * требуется бд об известном вредоносном трафике, бд регулярно обновляется.

  <details>
    <summary>Иллюстрация</summary>
    <br>
    <img width="1068" height="135" alt="image" src="https://github.com/user-attachments/assets/cb266c5f-0d81-4b12-9b30-89416e18e80b" />
  </details>

  2. **Anomaly-Based IPS** (на основе отклонений).

      * **обнаружение статических аномалий** (определеяем норму).
  ...
  <details>
    <summary>Иллюстрация</summary>
    <br>
    <img width="618" height="425" alt="image" src="https://github.com/user-attachments/assets/b7069b81-3e7f-4a5b-ac7b-125034932dfd" />
  </details>

  3. **Policy-Based IPS** (на основе политик).

      * требутеся бд политик. Реализация: firewall.

  <details>
    <summary>Иллюстрация</summary>
    <br>
    <img width="688" height="427" alt="image" src="https://github.com/user-attachments/assets/90fa421d-42d5-4c3b-bc92-8eb502355826" />
  </details>

  4. **Endpoint Security Controls** (контроль безопасности узлов).

  * Собственные элементы управления ОС и приложениями. На хостах устанавливается ПО агента:
  ...
  <details>
    <summary>Иллюстрация</summary>
    <br>
    <img width="280" height="497" alt="image" src="https://github.com/user-attachments/assets/bf55d043-24af-4bc8-a26a-e10d7ee90452" />
  </details>
</details>

<details>
  <summary>Методы обеспечения безопасности</summary>
  <br>

  1. Ориентированная на сеть:

      * защищённые сетевые устройства.
    
      * строгие правила firewall.
    
      * использование IPS в сети.
    
 2. Ориентированная на конечные устройства:

      * защищённая и обновленная ОС.
